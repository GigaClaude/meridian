...
RULES:
- Compress 5:1. 2000 tokens of input → 400 tokens of output.
- Only relevant facts. Tangential info = noise. Skip it.
- Preserve specifics: exact paths, ports, commands, rationale.
- Cite sources: [mem_xxx] after each fact.
- Newer memories override older ones. Flag conflicts.
- Shorter is ALWAYS better. Never pad to fill a budget.
- NEVER add section headers like "Summary:", "Warnings:", "Next Steps:", "Actionable Items:", "Deployment:". Write flowing prose, not structured reports.

FORMAT: Narrative prose with inline [mem_xxx] citations. For briefings: YAML only."""
...
class MemoryGateway:
    ...
    async def assemble_briefing(self, project_id: str, storage: StorageLayer) -> str:
        ...
        prompt = f"""Assemble a hot memory briefing from this raw data. Output ONLY valid YAML matching the hot memory schema. Keep it under 1500 tokens. Warnings go first.

{raw_context}

Output the YAML briefing now:"""
    ...
    async def recall(
        ...
        prompt = f"""Synthesize these search results into a direct answer.

RULES:
1. ONLY include facts that directly answer the query. Skip tangential results entirely.
2. Cite sources inline: "fact [mem_xxx]" after each claim.
3. Keep rationale: "Chose X because Y" not just "Uses X."
4. If memories conflict, ALWAYS prefer the newest one (check the age field). Flag it: "Note: [mem_a] (Xd old) says Y but newer [mem_b] (<1h) says Z — using newer."
5. Lead with the answer. Supporting details after.
6. Budget: {max_tokens} tokens MAX. Shorter is better.
7. NEVER add sections like "Warnings:", "Next Steps:", "Actionable Items:", "Deployment:", "Summary:". Just answer the query directly in flowing prose.
8. NEVER invent facts not in the search results.
9. Results marked VOLATILE contain ephemeral facts (ports, PIDs, temp paths). Note when citing volatile facts that they may have changed since last recorded.
